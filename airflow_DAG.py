...
client = boto3.client('emr', region_name='us-east-2',aws_access_key_id={'Your AWS KEY'},aws_secret_access_key={'Your AWS ACCESS KEY'})
...
def create_emr_cluster():
  cluster_id = client.run_job_flow(
    ...
    'Ec2SubnetId': 'subnet-0ac9f5822c2929eac',
    },
    LogUri="s3://rahul-kengeri-logs/logs/",
...
with dag:
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','s3://us-east-2.elasticmapreduce/libs/script-runner/script-runner.jar',[ 's3://data-jobs/scripts/ingest.sh']],
  dag=dag, 
  )
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','command-runner.jar',[ 'spark-submit',
            '--master', 'yarn',
            '--deploy-mode', 'cluster',
            's3://data-jobs/scripts/transform.py']],
  dag=dag, 
  )
